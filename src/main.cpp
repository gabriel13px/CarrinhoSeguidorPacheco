...
int ManualPid = 0;// 0 = PID, 1 = Manual
 ...
 uint8_t VelocidadeBaseA = 170;
 uint8_t VelocidadeBaseB = 170
;
...
void loop(){
  ...
  if(SerialBT.available()){
    ...
    if (KpIndex >= 0 && KiIndex > KpIndex && KdIndex > KiIndex && ManualPid==0) {
      String kpStr = EntradaSerial.substring(KpIndex+3, KiIndex - 1);
      String kiStr = EntradaSerial.substring(KiIndex + 3, KdIndex - 1);
  ...
  if(digitalRead(botao)==LOW && ManualPid == 0){
    OnOff = !OnOff;
    if(OnOff == true) {
